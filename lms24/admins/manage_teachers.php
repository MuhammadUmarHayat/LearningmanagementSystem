			...
			<tbody>
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					if ($row['status'] == "Pending") {
						echo "<td><a href='index.php?page=manage_teachers&Approve=Yes&ID=$row[ID]' class='btn btn-success btn-xs'>Approve User</a></td>";
					} else {
						echo "<td>
...
if (isset($_POST['submit'])) {
	...
	extract($_POST);
	if ($ID > 0) {
		$query = "UPDATE teachers SET name = '$name', email = '$email', address = '$address', cellNo = '$cellNo', qual = '$qual', password = '$password' WHERE ID = '$ID'";
		$msg = "Teacher Record Updated";
	} else {
		$query = "INSERT INTO teachers (name, email, address, cellNo, qual, password)
...
if (isset($_GET['Approve'])) {
	$ID = $_GET['ID'];
	$result = mysqli_query($con, "UPDATE teachers SET status = 'Approved' WHERE ID = '$ID'");
	if ($result) {
		echo "<script>
				window.location='index.php?page=manage_teachers&msg=Registration request approved';
			</script>";
	}
}
